...
test('system map, graph scenarios, keyboard inspection, and data export work', async ({ page }) => {
  ...
  const controls = page.getByRole('group', { name: 'Explore coordinated adjustments' });
  await expect(page.locator('.map-scope')).toContainText('Workload · Model · Runtime · Memory · Hardware');
  await controls.getByRole('button', { name: 'Kernels', exact: true }).click();
  await expect(page.locator('#control-description')).toContainText('memory layout');
  await expect(page.locator('.map-scope')).toHaveText('Spans Model · Runtime · Memory · Hardware');
  await expect(controls.getByRole('button', { name: 'Kernels', exact: true })).toHaveAttribute('aria-pressed', 'true');
  await controls.getByRole('button', { name: 'Hyperparameters', exact: true }).focus();
  await page.keyboard.press('Enter');
  await expect(page.locator('.map-scope')).toHaveText('Spans Workload · Model · Runtime · Memory');
  await controls.getByRole('button', { name: 'Scheduling', exact: true }).click();
  await expect(page.locator('.map-scope')).toHaveText('Spans Workload · Runtime · Memory · Hardware');
  await controls.getByRole('button', { name: 'Overview', exact: true }).click();
  await expect(controls.locator('[aria-pressed="true"]')).toHaveCount(1);
  await expect(page.locator('.map-scope')).toHaveText('Spans Workload · Model · Runtime · Memory · Hardware');
